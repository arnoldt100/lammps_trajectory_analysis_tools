...
# A C-contiguous N x 3 array of float types for reciprocal lattice wave vectors.
type WaveVectors16 = np.ndarray[tuple[int,Literal[3]],np.dtype[np.float16]]
type WaveVectors32 = np.ndarray[tuple[int,Literal[3]],np.dtype[np.float32]]
type WaveVectors64 = np.ndarray[tuple[int,Literal[3]],np.dtype[np.float64]]
WaveVectors = TypeVar('WaveVectors', WaveVectors16,WaveVectors32,WaveVectors64)
